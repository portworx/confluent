...
libmesos_bundle_url = 'https://downloads.mesosphere.com/libmesos-bundle/libmesos-bundle-1.10-1.4-63e0814.tar.gz'
...
'''
    ...
    def _get_file_confluent_kafka_mapping(self, filepath):
        '''Returns a confluent-kafka mapping (dict) for the following cases:
        ...
        - Custom environment params like 'CONFLUENT_KAFKA_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default confluent-kafka values (may be overridden via eg CONFLUENT_KAFKA_PACKAGE_VERSION envvars):
        confluent_kafka_mapping = {
        ...
        # look for any 'sha256:filename' confluent-kafka params, and get shas for those.
        # this avoids calculating shas unless they're requested by the confluent-kafka.
        ...
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            confluent_kafka_mapping['sha256:{}'.format(shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom CONFLUENT_KAFKA_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('CONFLUENT_KAFKA_'):
                # 'CONFLUENT_KAFKA_SOME_KEY' => 'some-key'
                confluent_kafka_mapping[env_key[9:].lower().replace('_','-')] = env_val

        return confluent_kafka_mapping
    ...
    def _apply_templating_file(self, filepath):
        ...
        confluent_kafka_mapping = self._get_file_confluent_kafka_mapping(filepath)
        orig_content = open(filepath, 'r').read()
        new_content = orig_content
        for confluent_kafka_key, confluent_kafka_val in confluent_kafka_mapping.items():
            new_content = new_content.replace('{{%s}}' % confluent_kafka_key, confluent_kafka_val)
        ...
        confluent_kafka_keys = list(confluent_kafka_mapping.keys())
        confluent_kafka_keys.sort()
        for key in confluent_kafka_keys:
            logger.info('  {{%s}} => %s' % (key, confluent_kafka_mapping[key]))
...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <package-version> <confluent-kafka-package-dir> <artifact-base-path> [artifact files ...]'.format(argv[0]))
    logger.info('  Example: $ {} kafka 1.2.3-4.5.6 /path/to/confluent-kafka/jsons/ https://example.com/path/to/kafka-artifacts /path/to/artifact1.zip /path/to/artifact2.zip /path/to/artifact3.zip'.format(argv[0]))
